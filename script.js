let add_to_cart_btns = document.getElementsByClassName('btn btn-light')
...
function updateTotal(event){
    ...
    price_field_content = price_field.children[0].innerText.replace('DT','')
    total_field.children[0].innerText = number_of_items.value * price_field_content + 'TND' 
...
function grandTotal(){
    ...
    for ( let i = 0; i < total_price.length; i++) {
       total_price_content = Number(total_price[i].innerText.replace ( 'TND' , '') )
       total += total_price_content
    }
    grand_total.children[0].innerText= total + 'TND'
